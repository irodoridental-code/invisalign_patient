...
export default function PatientForm({ doctors, onSubmit, onCancel, initial = null }) {
  ...
  const [chart, setChart] = useState(ip?.chart || '')
  const [dob, setDob]     = useState(ip?.dob === '—' ? '' : (ip?.dob || ''))
  const [type, setType]   = useState(ip?.type || 'adult')
  const [doc, setDoc]     = useState(ip?.doc || '')
  const [total, setTotal] = useState(ip?.phases?.[0]?.total || '')
  const [cur, setCur]     = useState(ip?.phases?.[0]?.cur || '')
  const [start, setStart] = useState(ip?.phases?.[0]?.start || '')
  const [at, setAt]       = useState(ip?.phases?.[0]?.at || '')
  const [cyc, setCyc]     = useState(ip?.phases?.[0]?.cyc || '7')
  ...
  function toggleIpr(stage) {
    setIprStages(prev => prev.includes(stage) ? prev.filter(s => s !== stage) : [...prev, stage].sort((a,b)=>a-b))
  }
    ...
    <div className="form-center">
      <div className="form-wrap">
        ...
        <div className="f-grid">
          <div style={{gridColumn:'1/-1'}}>
            ...
            <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
              <input className="f-inp" type="number" value={total}
                ...
                placeholder="20" min="1" max="60" style={{maxWidth:'140px'}} />
              <button className="btn btn-blue" style={{fontSize:'12px',padding:'7px 14px',whiteSpace:'nowrap'}} onClick={handleConfirmTotal}>決定</button>
          ...
          {confirmedTotal && (
            <div style={{gridColumn:'1/-1'}}>
              <label className="f-lbl" style={{marginBottom:'6px'}}>
                IPR処置ステージ <span style={{fontSize:'10px',color:'#888',fontWeight:400,marginLeft:'6px'}}>処置が必要なステージをタップして選択</span>
              </label>
              <div style={{display:'flex',flexWrap:'wrap',gap:'5px'}}>
                {Array.from({length:confirmedTotal},(_,i)=>i+1).map(stage => {
                  const sel = iprStages.includes(stage)
                  return (
                    <button key={stage} onClick={()=>toggleIpr(stage)} style={{
                      minWidth:'34px',height:'34px',padding:'0 4px',borderRadius:'6px',
                      border:sel?'2px solid #1d4ed8':'1px solid #ccc',
                      background:sel?'#dbeafe':'#f9f9f8',color:sel?'#1e40af':'#666',
                      fontWeight:sel?700:400,fontSize:'13px',cursor:'pointer',transition:'all .12s'
                    }}>{toC(stage)}</button>
                  )
                })}
              </div>
          ...
          {type === 'adult' && (
            <div>
              <label className="f-lbl">アタッチメントセット日（任意）</label>
              <input className="f-inp" type="date" value={at} onChange={e=>setAt(e.target.value)} />
            </div>
          )}
          <div style={{gridColumn:'1/-1'}}>
        ...
        <div style={{display:'flex',gap:'8px',marginTop:'12px'}}>
          <button className="btn btn-blue" onClick={handleSubmit}>{ip ? '変更を保存' : '患者を登録する'}</button>
